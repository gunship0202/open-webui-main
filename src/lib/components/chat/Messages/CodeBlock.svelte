<script lang="ts">
	...
	const generatePdf = async () => {
		...
		try {
			...
			toast.error('PDF 生成失败：' + error.message);
		} finally {
			generatingPdf = false;
		}
	};
		...
		{:else}
			...
			>
				<div class="flex items-center gap-0.5 translate-y-[1px]">
					...
					<button
						class="pdf-button bg-none border-none bg-gray-50 hover:bg-gray-100 dark:bg-gray-850 dark:hover:bg-gray-800 transition rounded-md px-1.5 py-0.5"
						on:click={generatePdf}
					>
						<div class="flex gap-1 items-center">
							<Document className="size-3" />
							<span>{generatingPdf ? $i18n.t('生成中...') : $i18n.t('生成PDF')}</span>
						</div>
					</button>
				</div>
			</div>
